...
export default function CrouselTemplate(props) {
    ...
    return (
        <div className={props.isDark === "true" ? 'mx-auto w-5/6  bg-gray-950 text-white' : 'mx-auto w-5/6  lg:my-10 '}>
            <h1 className='lg:mx-5  my-2 md:mt-5 font-bold text-2xl'>{props.heading}</h1>
            <Slider {...settings} className='lg:py-1 '>
                {images.map((image) => {
                    return (
                        <Link to={props.type === "tv" ? `/tv/${image.id}` : `/movies/${image.id}`} onClick={onTop} key={image.id} className='md:px-5 px-1 outline-none lg:w-64 transform transition-transform hover:translate-y-[-7px]'>
